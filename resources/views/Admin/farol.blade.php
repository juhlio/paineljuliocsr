    ...
    <table id="example" class="hover" style="width:100%">
        ...
        <tbody>
            ...
            <tr class="bg-danger">
                ...
                <td> <a href="https://app2.auvo.com.br/gerenciarEquipamentos/equipamento/{{$task->equipId}}" style="color: inherit; text-decoration: none;" target="_blank"> {{$task->equipId}} </a></td>
                <td> {{ $task->reply}}</td>
                <td> {{$task->razaoSocial}}</td>
                <td>{{$task->nome}}</td>
                <td>{{$task->nomeEquip}}</td>
                <td>{{$task->obs}}</td>
